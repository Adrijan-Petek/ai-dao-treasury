...
describe('DAOTreasury', function () {
  ...
  it('owner/relayer can propose and users can vote and execute when passed', async function () {
    // fund treasury
    await deployer.sendTransaction({to: dao.address, value: ethers.utils.parseEther('2')});

    // set relayer to deployer (owner by default)
    await dao.setRelayer(deployer.address);

    // create proposal
    const tx = await dao.propose(strat.address, '0x', 'test');
    await tx.wait();
    const proposalId = 1;
    ...
    // execute (send 1 ETH)
    await dao.executeProposal(proposalId, ethers.utils.parseEther('1'));
    // check treasury balance decreased
    const balanceAfter = await ethers.provider.getBalance(dao.address);
    expect(balanceAfter).to.be.lt(ethers.utils.parseEther('2'));
  });
});
